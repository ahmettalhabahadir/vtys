    ...
    public partial class studentpanel : Form
    {
        ...
        private void UpdatePermissionGrid()
        {
            try
            {
                ...
                string query = "SELECT permissionid AS \"İzin No\", startdate AS \"Başlangıç Tarihi\", enddate AS \"Bitiş Tarihi\", reason AS \"Açıklama\" " +
                               "FROM permissions WHERE tcnumber = @tcnumber ORDER BY startdate DESC";
        ...
        private void UpdateComplaintsGrid()
        {
            try
            {
                ...
                string query = "SELECT complaintdate AS \"Tarih\", description AS \"Açıklama\", status AS \"Durum\" " +
                               "FROM complaints WHERE tcnumber = @tcnumber ORDER BY complaintdate DESC";
        ...
        private void UpdateStudentInfoGrid()
        {
            try
            {
                ...
                string query = "SELECT tcnumber AS \"TC No\", name AS \"Ad\", surname AS \"Soyad\", phonenumber AS \"Telefon\", email AS \"E-posta\" " +
                               "FROM student WHERE tcnumber = @tcnumber";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Randevu kontrol fonksiyonunu çağır
                string functionCallQuery = "SELECT check_laundry_appointment(@laundryId, @date, @time)";
                NpgsqlCommand functionCallCommand = new NpgsqlCommand(functionCallQuery, baglanti);
                functionCallCommand.Parameters.AddWithValue("@laundryId", laundryId);
                functionCallCommand.Parameters.AddWithValue("@date", date);
                functionCallCommand.Parameters.AddWithValue("@time", time);

                bool isAppointmentAvailable = Convert.ToBoolean(functionCallCommand.ExecuteScalar());

                if (isAppointmentAvailable)
                {
                    MessageBox.Show("Bu makine, tarih ve saat için zaten randevu alınmış. Lütfen başka bir seçim yapın.");
                    return;
                }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // İzin ekleme sorgusu
                string insertQuery = "INSERT INTO permissions (tcnumber, startdate, enddate, reason) VALUES (@tcnumber, @startdate, @enddate, @reason)";
                NpgsqlCommand insertCommand = new NpgsqlCommand(insertQuery, baglanti);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string insertQuery = "INSERT INTO complaints (tcnumber, complaintdate, description, status) " +
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Güncelleme sorgusu
                string updateQuery = "UPDATE student SET phonenumber = @newPhoneNumber, email = @newEmail WHERE tcnumber = @tcnumber";
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Etkinliğe katılımı students_event tablosuna kaydedecek SQL sorgusu
