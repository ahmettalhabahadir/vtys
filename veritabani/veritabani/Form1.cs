    ...
    public partial class Form1 : Form
    {
        ...
        public void UpdateStudentGrid()
        {
            try
            {
                ...
                string sorgu = "SELECT tcnumber AS \"TC No\", name AS \"Ad\", surname AS \"Soyad\",phonenumber AS \"Tel No\", email, birthday AS \"Doðum Günü\", department AS \"Bölüm\", class AS \"Sýnýf\", roomno AS \"Oda No\", entryyear AS \"Giriþ Yýlý\" FROM student";

                NpgsqlDataAdapter dataAdapter = new NpgsqlDataAdapter(sorgu, baglanti); // Sorguyu çalýþtýr

                DataTable dataTable = new DataTable(); // Verileri tutmak için tablo oluþtur
                dataAdapter.Fill(dataTable); // Verileri tabloya doldur

                dataGridView1.DataSource = dataTable; // DataGridView'e tabloyu baðla
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Bir hata oluþtu: {ex.Message}"); // Hata mesajý göster
            }
            finally
            {
        ...
        public void UpdateStaffGrid()
        {
            try
            {
                ...
                // Tüm personel bilgilerini getir
                string sorgu = "SELECT tcnumber AS \"Personel TC\", name AS \"Ad\", surname AS \"Soyad\", role AS \"Görev\", phonenumber AS \"Telefon No\", entryyear AS \"Giriþ Yýlý\" FROM STAFF";
        ...
        private void UpdateComplaintsGrid()
        {
            try
            {
                // Veritabaný baðlantýsýný aç
                ...
                string query = "SELECT complaintid AS \"Þikayet ID\", tcnumber AS \"TC Numarasý\", complaintdate AS \"Þikayet Tarihi\", description AS \"Açýklama\", status AS \"Durum\" " +
                               "FROM complaints ORDER BY complaintdate DESC";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selectedOption == "Ýsim-Soyisim")
                {
                    ...
                    query = "SELECT * FROM STUDENT WHERE Name || ' ' || Surname ILIKE @nameSurname";
                }
                else if (selectedOption == "Oda No")
                {
                    if (int.TryParse(textBox2.Text, out int roomNo))
                    {
                        query = "SELECT * FROM STUDENT WHERE RoomNo = @roomNo";
                    }
                    else
                    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                baglanti.Open(); // Baðlantýyý aç
                string sorgu = "SELECT * FROM student"; // Öðrenci bilgilerini çekmek için SQL sorgusu
                NpgsqlDataAdapter dataAdapter = new NpgsqlDataAdapter(sorgu, baglanti); // Sorguyu çalýþtýr
            ...
            try
            {
                ...
                using (var transaction = baglanti.BeginTransaction())
                {
                    try
                    {
                        ...
                        // Eðer öðrenci baþarýyla silindiyse, iþlem tamamlanmýþ olur
                        if (rowsAffected > 0)
                        {
                        ...
                        {
                            // Eðer öðrenci bulunamazsa, iþlem iptal edilir
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // SQL sorgusu: TC numarasýna göre personeli sil
                string sorgu = "DELETE FROM STAFF WHERE tcNumber = @tcNumber";
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // SQL sorgusu: TC numarasýna göre personel bilgilerini al
                string sorgu = "SELECT * FROM STAFF WHERE tcNumber = @tcNumber";
        ...
        private void button17_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string updateQuery = "UPDATE complaints SET status = 'Completed' WHERE complaintid = @complaintId";
                NpgsqlCommand updateCommand = new NpgsqlCommand(updateQuery, baglanti);
        ...
        private void button18_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string tcNumber = textBox18.Text.Trim(); // Ziyaretçi TC Numarasý
                ...
                // TC numarasýnýn geçerli formatta olduðundan emin ol
                if (tcNumber.Length != 11 || !long.TryParse(tcNumber, out _))
                {
                    MessageBox.Show("Lütfen geçerli bir TC numarasý girin.");
                    return;
                }
